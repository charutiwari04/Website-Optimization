...
var resizePizzas = function(size) {
  ...
  var pizzaSizeElem = document.getElementById("pizzaSize").innerHTML;
  ...
  // Iterates through pizza elements on the page and changes their widths. Removed the 
  // previous determineDx function which was returning the size difference to change a pizza element from one size to another.
  // Included pizza size logic in same function which determines different pizza size.
  // Replaced the querySelectorAll with more efficient getElementsByClassName for selecting elements by class name.
...
// Changes done for improving performance issues:-
// 1. Moved the code for phase calculation into separate loop. Earlier it was within the main for loop which was doing layout changes.
// Recalulation of style and layout were happening more frequently which was causing performance issues. Anyway phase calculation is giving
// five different values then and now irrespective of how many loops are performed.
// 2. Replaced the element.style.left with element.style.transform, because this function only does composite changes whereas .left performs 
// layout, paint and composite, So transform saves time.
function updatePositions() {
  ...
  for (var i = 0; i < len; i++) {
	var check = items[i].basicLeft -650 + 100 * arr[i%5];
	items[i].style.transform= "translateX(" + check +"px)";
  }
...
// Generates the sliding pizzas when the page loads. Loads pizzas to div with ID movingPizzas1 in pizza.html.
// Reduced the pizza numbers to 50 from 200. 50 pizzas are enough to be visible on screen.
